...
%% Part 1 - Step 3: Thermal conductivities 
...
%% Part 1 - Step 4: The heat transport equation 

% Let's assume that the temperature of the upper surface(i.e. at z) is T
% and at the temperature at the lower surface (i.e. z+dz) is T+dT. Substitute these values
% into equation 1 (Think about how to replace d, T1, and T2). 
% In the limit that dz goes to zero, some part of the right-hand side of
% your new equation becomes a derivative. 
% Write this derivative.
% Q=-k([T+dT]-T/[Z-dZ]-Z)
% Write the heat flow (equation 1) using this derivative
% Q=-k(dt/dz)
% Write this same equation using the gradient operator
% Q=-kdT

%% Part 1 - Step 5: The conservation equation

% Use the heat transport equation and compute the derivative dQ/dz and 
% insert this into the conservation equation.
% (cpP)(dT/dt)=A-k(dT^2/dZ^2)
...
% The value of k is in m^2/s, k=(k/cpP)

%% Part 2: Oceanic lithosphere cooling 
%% Part 2 - Step 1: Setup the model domain and compute
%% Part 2 - Step 1: Nested for loop with erf function
tic
z = 1:100;
t = 1:100;
k = 31.556;
...
colormap(jet)
toc
%% Part 2 - Step 1: Nested for loop with erf function and Meshgrid
tic
[X,Y] = meshgrid(z,t);
T1 = [];
...
% The first figure took 1.34 seconds to run on TREX. The second figure
% takes 0.45 seconds to run. 

%% Part 2 - Step 2: Analyze the model output
...
% Depth and Time, but mostly time.
% How could we convert this model from age to distance from ridge axis?
% It could follow a similar gradient, only temperature would decrease with
% distance from axis.
...
% Why or why not?
% Yes, because the ridge is where the crust is cracked, and magma is
% pushing upwards. It is active tectonism. Without a high temperature to
% facilitate the excretion of mass, there would be no spreading.

%% Part 3: Plate velocty and the depth of oceans
%% Part 3 - Step 1: Load and plot sea-floor depth data
...
% List both oceans.
% I will finish this after work. crc

% Convert these plate veolocities to [cm/yr] and compare with an estimate
% from the literature from these ocean basins. Do your estimates agree with
% published Pacific and Atlantic Ocean spreading rates? 
% Gotta add these in

%% Part 3 - Step 2: A half-space model
% 
...
%% Part 4 - Step 1: Load topo data and plot sea-floor depths
...
%% Part 4 - Step 2: Kill the topography and get the units right
[iy, ix] = find(topo > 0);
...
%% Part 4 - Step 3 Compute sea-floor age
...
%% Part 4 - Step 5: Discuss your results
...
% Conforming to the model, the youngest ages occur along Mid-ocean ridges.
...
% sea as being ~20Ma, while accepted values are puting the
